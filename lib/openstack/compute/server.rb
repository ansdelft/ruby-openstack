...
module Compute
  class Server
    ...
    #   => array
    def interface_list
      response = @compute.connection.csreq("GET",@svrmgmthost,"#{@svrmgmtpath}/servers/#{ERB::Util.url_encode(self.id.to_s)}/os-interface",@svrmgmtport,@svrmgmtscheme,{'content-type' => 'application/json'})
      OpenStack::Exception.raise_exception(response) unless response.code.match(/^20.$/)
    ...
    def interface_details(id)
      response = @compute.connection.csreq("GET",@svrmgmthost,"#{@svrmgmtpath}/servers/#{ERB::Util.url_encode(self.id.to_s)}/os-interface/#{id}",@svrmgmtport,@svrmgmtscheme,{'content-type' => 'application/json'})
      OpenStack::Exception.raise_exception(response) unless response.code.match(/^20.$/)
    ...
    def interface_create(options)
      ...
      response = @compute.connection.csreq("POST",@svrmgmthost,"#{@svrmgmtpath}/servers/#{ERB::Util.url_encode(self.id.to_s)}/os-interface",@svrmgmtport,@svrmgmtscheme,{'content-type' => 'application/json'},data)
      OpenStack::Exception.raise_exception(response) unless response.code.match(/^20.$/)
      JSON::parse(response.body)["interfaceAttachment"]
    end
    ...
    def interface_delete(id)
      response = @compute.connection.csreq("DELETE",@svrmgmthost,"#{@svrmgmtpath}/servers/#{ERB::Util.url_encode(self.id.to_s)}/os-interface/#{id}",@svrmgmtport,@svrmgmtscheme,{'content-type' => 'application/json'})
      OpenStack::Exception.raise_exception(response) unless response.code.match(/^20.$/)
      true
    end
